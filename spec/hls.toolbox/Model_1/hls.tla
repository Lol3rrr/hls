...
\* BEGIN TRANSLATION (chksum(pcal) = "afb2fb18" /\ chksum(tla) = "528cf78f")
VARIABLES ServerWriteQueues, ServerReadQueues, ServerData, WrittenFiles, 
          ConfirmedFiles, OnlineServers, pc
...
vars == << ServerWriteQueues, ServerReadQueues, ServerData, WrittenFiles, 
           ConfirmedFiles, OnlineServers, pc, iteration, send_iteration, 
...
Run(self) == /\ pc[self] = "Run"
             ...
             /\ UNCHANGED << ServerWriteQueues, ServerReadQueues, ServerData, 
                             WrittenFiles, ConfirmedFiles, OnlineServers, 
                             iteration, write_request >>
...
IncRun(self) == /\ pc[self] = "IncRun"
                ...
                /\ UNCHANGED << ServerWriteQueues, ServerReadQueues, 
                                ...
                                OnlineServers, send_iteration, send_written_to, 
                                write_request >>
                         ...
                         /\ IF send_iteration[self] < Replicas
                               ...
                               ELSE /\ pc' = [pc EXCEPT ![self] = "IncRun"]
                                    /\ UNCHANGED << ServerWriteQueues, 
                                                    ...
                                                    send_iteration, 
                                                    send_written_to >>
                         /\ UNCHANGED << ServerReadQueues, ServerData, 
                          ...
                          /\ IF send_iteration[self] < Replicas
                                ...
                                ELSE /\ pc' = [pc EXCEPT ![self] = "IncRun"]
                                     /\ UNCHANGED << ServerReadQueues, 
                                                     send_iteration, 
                                                     send_written_to >>
                          /\ UNCHANGED << ServerWriteQueues, ServerData, 
                                          WrittenFiles, ConfirmedFiles, 
                                          OnlineServers, iteration, 
                                          write_request >>
                        ...
                        /\ IF ServerWriteQueues[self] # <<>>
                              ...
                              ELSE /\ TRUE
                                   /\ UNCHANGED << ServerWriteQueues, 
                                                   ...
                                                   write_request >>
                        /\ pc' = [pc EXCEPT ![self] = "RunServer"]
                        /\ UNCHANGED << ServerReadQueues, WrittenFiles, 
                                        OnlineServers, iteration, 
...
\* Last modified Wed Dec 21 15:46:55 CET 2022 by leon
